...
@Component(
  {
    selector: "padretutoridec",
    templateUrl: "./PadreTutor.template.html"
  })

export class PadreTutorIDECComponent
{
  //datos
  operacion: string;
  ...
  especificaescuela: boolean = false;
  ...
  SetPadreTutor(data)
  {
    ...
    tutor.Vive = !data.vive ? true : data.vive == "1" ? true : false;
  ...
  validarEscuela(id) {
    if (id && id == "1") {
      this.nuevoTutor.EspecificarAlumno = null;
      this.especificaescuela = true;
    } else {
      this.nuevoTutor.EspecificarAlumno = null;
      this.especificaescuela = false;
    }
  }
  ...
  EditarPadreoTutor() {
    this._httpService.putElemento('portalfamiliar/Ciencias/PadresOTutoresAlumno', this.SetPadreTutorGuardar(this.nuevoTutor)).subscribe(
      result => {
        if (result.status == 200) {
  ...
  SetPadreTutorGuardar(data) {
    ...
    tutor.Tutor = this.alumno.CambiarValorBoolAInt(data.Tutor);
    ...
    {
      tutor.Telefono = this._help.UnitTelefono(data.Lada, data.Telefono);
    }

    if (data.LadaEmpresa && data.TelefonoEmpresa)
    {
        tutor.TelefonoEmpresa = this._help.UnitTelefono(data.LadaEmpresa, data.TelefonoEmpresa);
    }
  ...
  ValidarDatosPadreOTutor(correoTutor, telefonoTutor, ladaTutor, fechaNacimiento, ladaEmpresa, telefonoEmpresa) {
    ...
    {
        if ((this.nuevoTutor.LadaEmpresa || this.nuevoTutor.TelefonoEmpresa || this.nuevoTutor.ExtensionEmpresa) &&   (!ladaEmpresa || !telefonoEmpresa))
        {
            return false;
        }
  ...
  //Cambio el valor de vive
  CambiarVive(e)
  {
    if(!e)
    {
      this.nuevoTutor.Tutor = false;
    }
  }
